...
class Tree(object):
    ...
    def innerReprWithParams(self, depth=""):
        if [] == self._children:
            return depth + str(self._root) + " " + str(self._PL.getRuleProb(self.getRoot())) + str(
                self.isComplete()) + "\n"
        res = depth + str(self._root) + " " + str(self._PL.getRuleProb(self.getRoot())) + str(
    ...
    def reprForMCSA(self, depth="", pos=1):
        ...
        if [] == self._children:
            return (
            depth + " <SM sid=\"" + sid + "\" svol=\"" + svol + "\" scd=\"" + scd + "\" did=\"" + did + "\" dvol=\"" + dvol + "\" dcd=\"" + dcd + "\" rvol=\"" + rvol + "\" rcd=\"" + rcd + "\" vol=\"" + vol + "\"/>\n",
            pos)
        ...
        if self.isLeaf() or type(self._rule) == type([]):  # If this node is leaf
            ...
            for child in expandableChildren:
                if withIndices:  # If you want the record of indices
                    ...
                    for frontierItem in child[0].getFrontier(
                            True):  # FrontierItem[0] is the node inside the child that can be expended
                    ...
                    childrenFrontiers.extend(
                        child.getFrontier())  # Add each child's frontier to the complete frontier
    ...
    def substitute(self, suggestedSubstituteLetter, childIndex):
        ...
        if (self.getChildByIndex(immediateChildIndex).substitute(suggestedSubstituteLetter, childIndex[
                                                                                            1:])):  # substitute according to child's rule
    ...
    def substituteNode(self, suggestedSubstituteNode, childIndex):
        ...
        # 2. It suits you and other children below you
        immediateChildIndex = int(childIndex[0])
        if (self.getChildByIndex(immediateChildIndex).substituteNode(suggestedSubstituteNode, childIndex[
                                                                                              1:])):  # substitute according to child's rule
            self.isComplete()  # Update that you are complete if needed
            return self.substituteDownwards(childIndex[0])  # substitute self and children
    ...
    def substituteDownwards(self, changedNodeIndex):
        ...
        if not (self.matchParameters(int(changedNodeIndex))):  # Update Self
            return False
        for child in self._children:
            success = child.substituteDownwards(
                '-1')  # Inform all children they need to update according to their rule too
            if not success:
                return False
