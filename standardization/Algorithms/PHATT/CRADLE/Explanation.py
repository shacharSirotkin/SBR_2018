...
class Explanation(object):
    ...
    def explanationAsXML(self,root):
        if self._trees == []:
            return "Empty Explanation"
        head = ET.SubElement(root, "Explanation")
        head.set("prob", str(self.getExpProbability()))
        treeNums = []
        for tree in self._trees:
            treeNums.append(tree.getID())
            treeXML = ET.SubElement(head, "Tree")
            treeXML.text = str(tree.getID())
            tree.treeAsXML(treeXML)
        return root
        # self.printExpProbability()

    def plotExplanation(self, listID, pp):
        labelsDict = {}
        G = nx.DiGraph()
        for tree in self._trees:
            tree.plotTree(listID, str(listID[0]), "explanation " + str(listID[0]), G, labelsDict)
        fig = plt.figure(figsize=(15, 15), dpi=100)
        nx.draw(G, pos=graphviz_layout(G, prog='dot'), node_size=1600, node_color=range(len(G)),
                prog='dot', labels=labelsDict, with_labels=True)
        pp.savefig(fig)
        plt.close()
