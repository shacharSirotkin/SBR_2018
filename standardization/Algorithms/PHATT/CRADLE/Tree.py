...
#         print "******************"
#         print self.reprWithParams()
#         print other.reprWithParams()
#         print "******************"        
        return True
    ...
    def asXML_TP(self, depth=""):
        if type(self.getRoot()._ch[0])==Sigma:
            return depth+"<"+self.getRoot()._ch+" />\n"
        
        res = depth+"<"+self.getRoot()._ch+">\n"
        for child in self._children:
            res += child.asXML_TP(depth=depth+"  ")
    ...
    def getLeaves(self):
        leavesArray = []
        if "A" in str(self._root) and self._children == []:
           leavesArray.append(self._root)
        else:
            for child in self._children:
                leavesArray.extend(child.getLeaves())
        return leavesArray
...
##########################################################################      
    ...
    def matchParameters(self, changedNodeIndex):
        ...
        if type(self._rule)==Rule.Rule:
            for constraint in self._rule._paramConst:
                if len(constraint)==4:
                    (i,iname,j,jname) = constraint 
                    ...
                    (i,iname,val) = constraint
                    if not self.setConstraintByVal(changedNode, i, val, iname):
                        return False
            return True
    ...
    def setConstraint(self, changedNode, toChangeIndex, changedParamName, toChangeParamName): 
        ...
        if valToAssign==None: #If parameters can agree
            ...
            # This is a helping function that takes a constraint and changes nodes accordingly
    def setConstraintByVal(self, changedNode, toChangeIndex, valToAssign, toChangeParamName): 
        toChange = self.getChildByIndex(toChangeIndex)
        oldParamValue = toChange._root.getParam(toChangeParamName)
        if (oldParamValue==None):       #if parameter is free for assignment
            toChange._root.setParam(toChangeParamName, valToAssign)
...
#             return True
    ...
    def treeAsXML(self, explanation):
        treeXML = ET.SubElement(explanation, str(self._root))
        #tree.set("prob", str(self._PL.getRuleProb(self.getRoot())))
        #tree.set("is complete", str(self.isComplete()))
        if [] == self._children:
            return treeXML
        for child in self._children:
            child.treeAsXML(treeXML)
        return treeXML


    def plotTree(self, listID, parent, labelOfParent, G, labelsDict):
        listID[1] += 1
        node1 = parent + " " + labelOfParent
        parent = str(listID[0]) + " " + str(listID[1])
        node2 = parent + " " + str(self._root)
        labelsDict[node1] = labelOfParent
        labelsDict[node2] = str(self._root)
        labelOfParent = str(self._root)
        G.add_node(node1)
        G.add_node(node2)
        G.add_edge(node1, node2)
        if [] != self._children:
            for child in self._children:
                child.plotTree(listID, parent, labelOfParent , G, labelsDict)