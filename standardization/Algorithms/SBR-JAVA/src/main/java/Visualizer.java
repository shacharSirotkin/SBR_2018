...
public class Visualizer {
    ...
    public static void makeTreeGraph(TreeNode finalRoot, String fileName) throws DocumentException, IOException {
        ...
        for (TreeNode expRoot : finalRoot.getChildren()) {
            DelegateTree<TreeNode, Pair<TreeNode, TreeNode>> g = new DelegateTree<>(new DirectedOrderedSparseMultigraph<TreeNode, Pair<TreeNode, TreeNode>>());
            g.addVertex(expRoot);
    ...
    private static void printGraphToPDF(List<DelegateTree<TreeNode, Pair<TreeNode, TreeNode>>> trees, String fileName) throws IOException, DocumentException {
        ...
        for (int i = 0 ; i < trees.size() ; i++) {
            TreeLayout<TreeNode, Pair<TreeNode, TreeNode>> layout1 = new TreeLayout<TreeNode, Pair<TreeNode, TreeNode>>((DelegateTree<TreeNode, Pair<TreeNode, TreeNode>>)trees.get(i));
            ...
            VisualizationImageServer<TreeNode, Pair<TreeNode, TreeNode>> vis = new VisualizationImageServer<TreeNode, Pair<TreeNode, TreeNode>>(vv.getGraphLayout(), vv.getGraphLayout().getSize());
            if (i == 0)
            {
                document.open();
                document.setPageSize(new com.itextpdf.text.Rectangle((vis.getWidth() + 20)*10, (vis.getHeight() + 20)*10));
            }
